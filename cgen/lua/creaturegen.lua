...
function scan(creature, data)
	...
	for i = #data, 1, -1 do
		if data[i]:len() == 0 or data[i]:match('^[-]$') then
			table.remove(data, i);
...
function parseDefense(creature, data)
	...
	if regen then
		creature.sqline = creature.sqline .. ' regeneration ' .. trim(regen):gsub(';', '') .. '; ';
	end
	if fh then
		creature.sqline = creature.sqline .. ' fast healing ' .. trim(fh):gsub(';', '') .. '; ';
	end
	if defab then
		creature.sqline = creature.sqline .. 'Defensive Abilities ' .. trim(defab):gsub(';', '') .. '; ';
	end
	if sq then
...
function formatSuperSubScript(str)
	...
	local retval = str;
	local cases = {'1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th', '9th', 'CR', 'UM', 'TG', 'UC', 'APG', 'ACG', 'MC', 'HA', 'OA', 'MA', 'UI', 'UE', 'GMG', '*', 'B2', 'B3', 'B4', 'B5', 'B6', 'D'};
...
function parseStatistics(creature, data)
	...
	if lang then
		creature.lang = trim(getValueByName('Languages', lang, {}));
	end
...
function parseEcology(creature, data)
	...
	if creature.mark_ecology then
		...
		for i = (math.min(creature.mark_special_abilities + 1, creature.mark_ecology + 1)), #data do
			line = data[i];
			lline = data[i]:lower();
			if lline == 'special abilities' then
				break
			end

			if lline:match('%(su%)') or lline:match('%(ex%)') or lline:match('%(sp%)') or lline == 'ecology' or line:match('Environment') or line:match('Organization') or line:match('Treasure') then
				-- skip
			else
...
--[[
    Extract Name and value for special ability
]] --
function extractSpecialAbilitiesNameValue(ab, line, data, linenumber)
...
function getLineByName(strName, aryLines, locStart, locEnd)
	...
	if (not strName or not aryLines) then
		return nil;
	end
...
function getValueByName(strName, strLine, termChars)
	if (not strLine or not strName or not termChars) then
		return nil
	end
	local retval;
	local loc = -1;
